...
#(pdf-normalized) Gaussian for plotting shape in 1(b), and integrating in 1(c)
...
def mykstest(data,cdf=GaussianCDF):  
    ...
    for i in range(len(data_sorted)):
        ...
        fn=(i+1.)*N**-1.
        cdfeval=cdf(data_sorted[i]) # calculate cdf point
...
#area under Kolmogorov PDF (of V-statistic) until x for finding p-value as in Press et al. (a.k.a. the complementary CDF)
def Q_KP(l): 
        if l<0.0:   # filter for the value of z corresponding to a real cdf
            return np.nan
        if l==0.0:
            return 0.0
        else:        # otherwise calculate the actual p value(=1-P(z)) at z
            expterm=exp(-2.*l**2.) 
            p=2.*((4.*l**2.-1)*expterm-(16.*l**2.-1)*expterm**4.+(36.*l**2.-1)*expterm**9.) #just calculate to first few terms
            return p
...
def kuiperstest(data,cdf=GaussianCDF):  # (defaults to test of standard normal)
    ...
    for i in range(len(data_sorted)):
        fo=i*N**-1.      # counters to find distances relative to the CDF
        fn=(i-1.)*N**-1. #!!! different sign than mykstest()
        cdfeval=cdf(data_sorted[i]) # calculate cdf point
        ...
        V= mostpositive+mostnegative
        pval= Q_KP( (N**0.5+0.155+0.24*N**-0.5)*V )  # calculate corresponding p-value for this random observation of V = D- + D+ (Press: Eqtn. 14.3.25)
    return mostpositive,mostnegative,pval
